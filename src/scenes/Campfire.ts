...
export class CampfireScene extends Phaser.Scene {
  ...
  private drawCell(): void {
    ...
    fireGlow.fillCircle(GAME_W / 2, 332, 126);
    this.tweens.add({
      targets: fireGlow,
      alpha: 0.45,
      duration: 760,
    ...
    const hero = this.add.image(GAME_W / 2 - 126, 344, 'hero_down_0').setScale(5).setDepth(3);
  ...
  private drawFire(): void {
    ...
    logs.lineBetween(GAME_W / 2 - 42, 362, GAME_W / 2 + 38, 376);
    logs.lineBetween(GAME_W / 2 + 42, 362, GAME_W / 2 - 38, 376);
    logs.fillStyle(0x26160f, 1);
    logs.fillEllipse(GAME_W / 2, 372, 106, 18);

    const outer = this.add.graphics().setDepth(4);
    outer.fillStyle(0xff7a2f, 0.95);
    outer.fillTriangle(GAME_W / 2 - 28, 363, GAME_W / 2, 281, GAME_W / 2 + 24, 363);

    const inner = this.add.graphics().setDepth(5);
    inner.fillStyle(0xf1c40f, 0.96);
    inner.fillTriangle(GAME_W / 2 - 14, 365, GAME_W / 2 + 10, 303, GAME_W / 2 + 24, 365);
  ...
  private redraw(): void {
    ...
    for (const [index, purchase] of CAMPFIRE_PURCHASES.entries()) {
      this.addPurchaseButton(purchase, GAME_W / 2, startY + index * 56);
    }

    this.dynamic.add(this.add.text(90, 438, this.pendingPrepSummary(), {
      ...TEXT_STYLE,
      fontSize: '14px',
      color: '#b8b0c8',
      lineSpacing: 8,
