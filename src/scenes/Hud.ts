...
export class HudScene extends Phaser.Scene {
  ...
  private redraw(): void {
    ...
    for (let i = 0; i < 10; i++) {
      ...
      const img = this.add.image(28 + (i % 5) * 26, y0 + 24 + Math.floor(i / 5) * 24, full ? 'heart' : 'heart_empty');
      img.setScale(2.4);
      this.dynamic.add(img);
    }
    this.dynamic.add(
      this.add.text(28, y0 + 78, `HP ${Math.max(0, run.hp)}/${run.maxHp}`, {
        fontFamily: 'monospace', fontSize: '13px', color: '#d8d2e4',
      }),
    );
    ...
    for (const [i, card] of run.combatHand.entries()) {
      ...
      this.dynamic.add(g);
      this.dynamic.add(
        this.add.text(x + 28, y0 + 44, card.name.split(' ')[0], {
          fontFamily: 'monospace', fontSize: '9px', color: '#f5edd8',
        }).setOrigin(0.5),
      );
      this.dynamic.add(
      ...
      this.dynamic.add(
        this.add.text(x + 28, y0 + 84, card.type.toUpperCase(), {
          fontFamily: 'monospace', fontSize: '8px', color: '#b8b0c8',
        }).setOrigin(0.5),
      );
    }
    ...
    this.dynamic.add(
      this.add.text(invX, y0 + 94, run.scoutCharges > 0
        ? `SCOUT x${run.scoutCharges}  [P] potion  [C] cards`
        : '[P] drink potion  [C] cards', {
        fontFamily: 'monospace', fontSize: '9px', color: run.scoutCharges > 0 ? '#f1c40f' : '#6a6478',
      }).setOrigin(0, 0.5),
    );
